    ...
    <div className="flex flex-col w-full p-4 sm:p-6 gap-6 sm:gap-8 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div className="space-y-1.5">
          <div className="flex items-center gap-2">
            <span className="font-mono text-xs text-primary uppercase font-bold tracking-wider">Quality Assurance &amp; AI Vision</span>
            <span className="w-1.5 h-1.5 rounded-full bg-primary" />
            <span className="text-xs text-on-surface-variant font-medium">Kiểm định chất lượng</span>
          </div>
        ...
        <button
          ...
          disabled={scanning}
          className="bg-primary text-on-primary px-5 py-2.5 rounded-xl font-semibold shadow-md shadow-primary/20 hover:bg-on-primary-fixed-variant transition-all flex items-center gap-2 text-xs uppercase tracking-wider font-mono w-fit"
        >
          <Upload size={16} className={scanning ? 'animate-spin' : ''} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 stagger">
        <div className="bg-surface-container-lowest p-6 shadow-sm border border-outline-variant/40 border-l-4 border-l-primary rounded-2xl card-hover">
          <p className="font-mono text-xs text-on-surface-variant uppercase font-semibold">Tỷ lệ lỗi toàn diện (YTD)</p>
          <span className="text-4xl font-extrabold text-on-surface mt-2 block font-mono">1.42%</span>
          <p className="text-xs text-emerald-600 mt-1 font-semibold">-0.3% so với mục tiêu quý</p>
        </div>

        <div className="gradient-primary p-6 shadow-xl rounded-2xl text-on-primary card-hover flex justify-between items-start">
          <div>
            <p className="font-mono text-xs opacity-80 uppercase font-semibold">Lỗi nghiêm trọng cần xử lý</p>
            <span className="text-4xl font-extrabold mt-2 block font-mono">04 <span className="text-lg font-normal opacity-80">vụ</span></span>
            <p className="text-xs opacity-90 mt-1">Đã khoanh vùng cách ly cuộn lỗi</p>
          </div>
          <div className="p-3 rounded-2xl bg-on-primary/15">
        ...
        <div className="bg-surface-container-lowest p-6 shadow-sm border border-outline-variant/40 border-l-4 border-l-sky-500 rounded-2xl card-hover">
          <p className="font-mono text-xs text-on-surface-variant uppercase font-semibold">Thời gian phản hồi TB</p>
          <span className="text-4xl font-extrabold text-on-surface mt-2 block font-mono">18 <span className="text-lg font-normal text-on-surface-variant">phút</span></span>
          <p className="text-xs text-on-surface-variant mt-1">Từ lúc phát hiện đến khi hiệu chỉnh máy</p>
        </div>
      </div>
      ...
      <div className="bg-surface-container-lowest shadow-sm border border-outline-variant/50 rounded-2xl overflow-hidden animate-slide-up">
        <div className="p-4 sm:p-5 border-b border-outline-variant/40 bg-surface-container-low/60 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="text-base font-bold text-on-surface">Danh mục Khuyết tật Bề mặt &amp; Cơ tính</h3>
          </div>
          ...
          <table className="w-full text-left min-w-[550px]">
            <thead>
              <tr className="bg-surface-container-low/40 border-b border-outline-variant/30">
                ...
                <th className="px-5 py-3.5 font-mono text-xs text-on-surface-variant uppercase font-semibold">Loại lỗi</th>
                <th className="px-5 py-3.5 font-mono text-xs text-on-surface-variant uppercase font-semibold">Tần suất xuất hiện</th>
                <th className="px-5 py-3.5 font-mono text-xs text-on-surface-variant uppercase font-semibold text-right">Mức độ cảnh báo</th>
              </tr>
            </thead>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-on-surface">Thư viện Nhận dạng Khuyết tật AI Vision</h3>
          </div>
          <span className="text-xs text-on-surface-variant font-mono">Dataset: 250,000+ ảnh bề mặt</span>
        </div>
            ...
            >
              <div className="relative aspect-video overflow-hidden bg-surface-container-highest">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 via-black/40 to-transparent text-white">
                  <span className="font-mono text-[10px] px-2 py-0.5 rounded bg-primary text-on-primary font-bold uppercase mr-2">
                    {item.code}
                  </span>
              ...
              <div className="p-4 border-t border-outline-variant/30 flex items-center justify-between bg-surface-container-low/40">
                <div className="text-xs text-on-surface-variant truncate pr-2">
                  <span className="font-semibold text-on-surface">Khuyến nghị AI:</span> {item.recommendation}
                </div>
                <button
                  onClick={() => {
                    addToast({
                  ...
                  className="px-3 py-1.5 rounded-xl bg-surface-container hover:bg-surface-container-high text-xs font-semibold text-primary transition-colors shrink-0"
                >
                  Xem quang phổ
                </button>
              </div>
