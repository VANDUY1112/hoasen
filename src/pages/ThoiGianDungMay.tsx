      ...
      <section className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div className="space-y-1.5">
          <div className="flex items-center gap-2">
            <span className="font-mono text-xs text-primary uppercase font-bold tracking-wider">Live Downtime Telemetry</span>
            <span className="w-1.5 h-1.5 rounded-full bg-primary" />
            <span className="text-xs text-on-surface-variant font-medium">Gián đoạn dây chuyền</span>
          </div>
        ...
        <div className="bg-surface-container-lowest p-5 rounded-2xl shadow-sm border border-outline-variant/40 flex items-center gap-4 w-full sm:w-auto">
          <div className="p-3 rounded-2xl bg-rose-500/10 text-rose-600">
            <TimerOff size={24} />
          </div>
          <div>
            <p className="font-mono text-[11px] text-on-surface-variant uppercase font-semibold">Tổng thời gian dừng ca</p>
            <p className="text-3xl font-extrabold text-primary font-mono leading-none mt-0.5">
              142 <span className="text-sm font-medium text-on-surface-variant">phút</span>
            </p>
          </div>
        </div>
      </section>
            ...
            <tbody className="divide-y divide-outline-variant/30 text-sm">
              {filtered.map((r, i) => (
                <tr key={i} className="hover:bg-surface-container/60 transition-colors group">
                  <td className="px-5 py-4 font-mono text-xs font-medium text-on-surface-variant">
                    {r.time}
                  </td>
